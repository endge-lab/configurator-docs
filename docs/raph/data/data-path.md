...
`DataPath` только описывает selector и не хранит индекс. При использовании `DefaultDataAdapter` поиск по `[key=value]` по умолчанию ускоряется отдельным индексом массива, который строится при первом обращении. Стратегии, жизненный цикл и ограничения разобраны на странице [Индексация коллекций](/raph/data/indexing).
