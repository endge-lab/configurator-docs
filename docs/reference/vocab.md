...
Ручной вызов нужен, когда порядок определяет приложение. Например, после
авторизации приложение может сначала загрузить только `workspace`; остальные
справочники не должны автоматически подгружаться до активации соответствующей
Composition.
...
identity `catalog-airlines`. Mapping также статичен и задаёт пути внутри одной
