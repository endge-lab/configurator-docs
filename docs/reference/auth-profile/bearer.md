...
`config` должен быть пустым объектом; `session` не указывается. Подготовьте переменную `API_TOKEN` и выберите `api-bearer` в политике авторизации запроса. Общие правила см. в разделе [AuthProfile](../auth-profile).
...
Service Backend Endge в OIDC-режиме принимает только JWT, прошедший его проверки issuer, подписи, audience и срока действия. Возможность передать любую строку через Bearer-профиль не означает, что backend её примет. Настройки проверяющего backend описаны на странице [OIDC](../../configurator/authentication/oidc).
