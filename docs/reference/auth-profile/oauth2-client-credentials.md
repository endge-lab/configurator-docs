...
`clientId`, `scopes` и URL условные. Список `scopes` может быть пустым, если это допускает провайдер. Подготовьте переменную `API_CLIENT_SECRET` и выберите профиль `service-api` в запросе. Правила credentials и хранения описаны в разделе [AuthProfile](../auth-profile).
