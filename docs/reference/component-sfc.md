...
`$data.metaOf(reference, namespace?)` связывает template с Raph Meta-plane
входного значения. Это не metadata компонента или ячейки: адрес выводится из
provenance входного prop, а изменение Meta реактивно инвалидирует Component
runtime. Полный контракт и пример optimistic status приведены в разделе
[контекстных переменных](/sfc-tables/context-variables#метаданные-входных-данных).
