...
Для editable-примитивов `Text`, `Number` и `DateTime` AODB adapter по умолчанию использует compact editor:
сохраняет типографику отображаемого значения, убирает у редактора фон, рамку, shadow, внутренние отступы
и системные controls. Текст и число занимают ширину своего содержимого, а редактор `HH:mm` сохраняет ширину
времени. Для пустого editable `DateTime` адаптер оставляет невидимую область размером с `HH:mm`, поэтому двойной
клик может начать ввод нового значения.

Явный `editor-variant="default"` возвращает прежний полноразмерный editor. `editor-variant="compact"` остаётся
допустимым renderer-neutral hint и позволяет явно зафиксировать compact-режим. Адаптеры без поддержки вариантов
используют собственный обычный редактор.
