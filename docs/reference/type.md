...
`source` и legacy `schema` пока сохраняются одновременно, но независимо. Изменение Source не обновляет Legacy Form, а изменение Legacy Form не переписывает Source. Пустой Source у существующего legacy-типа является допустимым состоянием.
:::
...
Имя в `field('...')` — identity примитивного или пользовательского Type. Импорты для пользовательских типов не требуются.
...
`unionOf` requires at least two unique type expressions. `type(identity)` создаёт ссылку на другой доменный Type, а `objectOf(...)` добавляет anonymous variant inline.
...
Monaco показывает parser diagnostics непосредственно в Source. Для ссылки `field('CustomType')` или `type('CustomType')` командой `Cmd+B` на macOS либо `Ctrl+B` на Windows/Linux можно открыть связанный Type document.
...
Core сохраняет `source` отдельно и не вкладывает его внутрь legacy `schema`. Automatic synchronization and backfill are intentionally absent until the next migration stage.
