...
На этой странице показано, как конфигурационная модель становится работающим сценарием. Общая идея описана на странице [«Что такое Endge»](/), а настройка, диагностика и проверка сценариев — на странице [«Единая среда работы»](/getting-started/shared-workspace).
...
| --- | --- |
| **Type** | Структуру данных и типы полей |
| **Query** | Запрос, его параметры и выходные данные |
| **Component SFC** | Структуру компонента, свойства и взаимодействия |
| **Store** | Организацию состояния |
| **DataView** | Преобразование и представление данных |
| **Action** | Выполняемые операции |
| **Composition** | Связи элементов и условия их запуска |

Например, в списке заявок Query описывает получение записей, Store содержит результат, а Component SFC представляет его пользователю. Composition связывает эти части. Один документ можно использовать в разных композициях, если его контракт подходит каждому сценарию. Полная карта описаний доступна в разделе [«Сущности Endge»](/domain/entities).
...
Контекст связывает запуск с Workspace и выбранными документами динамических фасетов:

- **Workspace** — рабочее пространство команды;
- **Facet** — определённая автором ось контекста, например заказчик, регион или стадия;
- **Facet Document** — выбранное значение этой оси и его configuration contribution.

Эти уровни участвуют в разрешении effective configuration. Сначала применяется
Workspace, затем выбранный документ каждого активного фасета по сохранённой позиции;
правила наследования и замены описаны в [справочнике Configuration](/reference/configuration).
...
## Проверка сценария на подготовленных данных

Runtime Preview позволяет исследовать поддерживаемый сценарий в конфигураторе. Для повторяемых примеров можно подготовить Mock-документы, mock-ответы Query и входные значения preview. Общий режим Mock/Live задаётся в контексте; локальные настройки сценария могут иметь приоритет.

Например, список заявок можно проверить на пустом ответе и большом наборе строк, сохраняя его связи между запросом, состоянием и интерфейсом. Такая проверка показывает поведение конкретного запуска; автоматическое сравнение с ожидаемым результатом требует отдельного механизма тест-кейсов.

Simulation хранит декларативное описание подмен для одной Composition. Его компиляция проверяет описание и зависимости, но не запускает симуляцию. Доступные способы проверки и направление развития тест-кейсов описаны в [единой среде работы](/getting-started/shared-workspace#тест-кеисы-и-симуляции).
