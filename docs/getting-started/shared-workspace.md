...
Конфигуратор Endge объединяет работу с конфигурационной моделью: от редактирования документов и настройки контекстов до проверки связей и исследования исполнения. Команда получает общее рабочее пространство для тех частей экосистемы, которые используют поддерживаемые конфигурации Endge.
...
| **Администратор** | Управляет доступными ему настройками, вариантами контекста и подключениями в пределах своих полномочий |
...
## Настройка вариантов приложений

В одной экосистеме могут использоваться общие конфигурации и разные значения для конкретных запусков. Например, один вариант обращается к тестовому сервису, а другой — к рабочему. Конкретные измерения различий определяет сам Workspace: это могут быть организация, продукт, регион, стадия поставки или другие фасеты.

Endge учитывает Workspace и выбранный документ каждого активного фасета в заданном порядке. Это позволяет задавать общие значения и явные переопределения без фиксированного набора уровней. Конфигуратор строит элементы выбора по данным домена, а Core получает итоговую конфигурацию каскадом `Workspace → facet selections`.
