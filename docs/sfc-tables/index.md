...
- [Контекстные переменные](./context-variables) описывают `$table`, `$row`, `$column`, `$cell`, `$context` и `props`.
- Меню ячеек и заголовков имеют разные context и описаны отдельно.
