...
- **Вызов из любого места**: один API, например `modals.open('create-document', { collection: 'queries' })` или `modals.open(ModalId.CreateVersion, { ... })`. Состояние «какой модал открыт и с какими параметрами» хранится централизованно (например, в слое/сторе модалов); один контейнер в корне приложения (или в layout) рендерит текущий модал по данным из стора.
