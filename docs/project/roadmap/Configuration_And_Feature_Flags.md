...
В enterprise и Low-Code платформах deployment configuration (URL, флаги фич,
режимы) выносится в единую точку входа с типизацией и валидацией при старте. Это
снижает ошибки в env и упрощает поддержку remote config. Риск при отсутствии:
разрозненные чтения `import.meta.env` и невозможность централизованно включить
фичу для конкретного динамического контекста без пересборки.
...
- Плюсы: просто, без сервера. Минусы: только build-time, нет различий по facet selections без пересборки и нет remote config.

### Domain configuration

- **Endge**: Workspace configuration и contributions выбранных facet documents
  разрешаются в `Endge.configuration.current`. Это конфигурация приложения с точки
  зрения Domain, а не инфраструктурные флаги и env.
...
### Соотношение с Domain configuration

- **Domain configuration** — authored значения Workspace и документов динамических фасетов, которые использует runtime и business logic.
- **Host config** — deployment env, feature flags, URLs и режимы отладки. Он не подменяет Domain configuration: host сначала читает собственные inputs, затем Core загружает Domain и разрешает effective configuration.
- Одно значение не должно иметь два независимых source of truth. Если host input участвует в Domain, его mapping оформляется явным boot contract.
...
- Domain configuration Endge не заменяется: host config относится к deployment-платформе и feature flags. Переопределение переменных через env описано в **Variables_Env_Override**.
