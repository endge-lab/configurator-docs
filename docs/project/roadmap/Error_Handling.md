...
3. **Error boundary по поддеревьям**
   - Во Vue 3 нет встроенного Error Boundary как в React; реализуется обёрткой-компонентом с `onErrorCaptured` и слотом fallback. Имеет смысл такой компонент ввести (например, в @endge/vue) и использовать для виджетов админки и тяжёлых секций, чтобы падение одного блока не показывало полноэкранную ErrorView.
