...
- В домене упоминается Policy (документ/сущность); в roadmap backend’а — политики на Workspace, папки и CRUD документов. На фронте нет выделенного модуля проверки политик: решения размазаны по компонентам и роутеру.
...
- **Политики**: правила вида «роль/группа может выполнять действие над ресурсом (или типом ресурса) в scope (Workspace, folder, facet)». Вычисление на бэкенде; фронт либо получает «разрешённые действия» для текущего контекста (страница, документ), либо отправляет запрос «can I do X on Y» и получает да/нет.
...
- **Интеграция с Endge**: при загрузке Workspace Domain подтягивать политики для текущего пользователя и scope; runtime и UI обращаются к одному фасаду, а не к разным местам с ролями.
...
- **Формат записи**: кто (userId, sessionId), что (action, resourceType, resourceId), когда (timestamp), контекст (workspace, facet selections, folder, IP, user agent при необходимости), опционально — до/после для изменений.
...
3. **Общая модель**
   - Использовать единые идентификаторы пользователя, сессии и контекста (Workspace, facet selections, folder) и для проверки прав, и для подписи записей аудита.
