...
Общий Grid Layout размещает штатный виджет **Домен** со всеми документами
...
Footer показывает выбранные документы динамических фасетов, язык, тему и часовой пояс клиента.
...
один dynamic facet, user, theme, timezone и data-mode.
...
Configurator использует это для переключения выбранного документа фасета с
