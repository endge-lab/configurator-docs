...
В нашем примере команда задаёт запрос к сервису 1, связи компонентов 1 и 2, а также использование настроек контекста. Сохранение этих документов фиксирует описание. Получение данных происходит при выполнении соответствующего сценария.
...
Runtime создаёт экземпляры подготовленного сценария и управляет их состоянием и ресурсами. В нашем примере компонент 1 передаёт параметры и запускает запрос; полученные данные попадают в состояние и отображаются компонентом 2.
...
Далее: [как команда работает с моделью в конфигураторе](/product/workspace). Подробности — в [жизненном цикле документов](/domain/lifecycle).
