...
Для одного контекста требуется изменить состав полей компонента 2. Команда сначала выясняет, предусмотрена ли соответствующая настройка и доступны ли нужные данные. Если возможность уже есть, достаточно настроить вариант. Если её нет, разработчик расширяет соответствующий контракт и реализацию.
...
Для сценария получения и отображения данных полезно проверить пустой ответ, несколько типовых записей и большой набор строк. Mock-документы и поддерживаемые mock-настройки Query позволяют подготовить данные, а Runtime Preview — исследовать конкретный запуск.

Команда проверяет, как отображаются поля, что происходит при изменении параметров компонента 1 и какие данные получает компонент 2. Модель сценария сохраняет свои связи; выбранный mock-путь меняет получение данных в пределах поддерживаемого контракта.
...
Далее: [как конфигуратор связан с приложениями и сервисами](/product/ecosystem).
