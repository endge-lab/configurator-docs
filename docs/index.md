...
Рассмотрим два способа организовать описания внутри одной микросервисной системы. В обоих случаях приложения и сервисы остаются самостоятельными, со своими задачами и программными реализациями.

### Описания внутри каждого приложения

В первом случае общие решения выражены отдельно в коде каждого потребителя. Например, фронтенды описывают данные заявки и запросы к сервису, а бэкенд — соответствующие структуры и API-контракты. Это могут быть разные языки и формы записи одного требования.

```mermaid
flowchart TB
  CHANGE["Изменение общего требования"] --> D1
  CHANGE --> D2
  CHANGE --> D3

  subgraph FRONT_A["Фронтенд A"]
    D1["Описание данных<br/>и запросов"] --> I1["Собственная<br/>реализация"]
  end
  subgraph FRONT_B["Фронтенд B"]
    D2["Описание данных<br/>и запросов"] --> I2["Собственная<br/>реализация"]
  end
  subgraph BACK["Бэкенд"]
    D3["Описание данных<br/>и API-контрактов"] --> I3["Собственная<br/>реализация"]
  end

  class CHANGE endgeInput
  class D1,D2,D3 endgeRegistry
  class I1,I2,I3 endgePackage
```

Стрелки показывают, какие описания затрагивает изменение требования. Команде нужно согласованно обновлять их и проверять совместимость. Такая организация не обязательна для микросервисов: общие библиотеки, схемы и генерация уже позволяют уменьшить повторение. Однако состав прикладного сценария и связи между его частями часто продолжают поддерживаться отдельно.

### Общая модель и совместимые потребители

Endge позволяет вынести поддерживаемые описания данных, операций и взаимодействий в общую конфигурационную модель. Каждый потребитель использует нужную ему часть через совместимый runtime, библиотеку или адаптер.

```mermaid
flowchart TB
  MODEL["Общая конфигурационная модель<br/>данные, контракты и сценарии"] --> R1
  MODEL --> R2
  MODEL --> R3

  subgraph FRONT_A["Фронтенд A"]
    R1["Endge Core<br/>и UI-адаптер"] --> I1["Собственные компоненты<br/>и операции"]
  end
  subgraph FRONT_B["Фронтенд B"]
    R2["Endge Core<br/>и UI-адаптер"] --> I2["Собственные компоненты<br/>и операции"]
  end
  subgraph BACK["Совместимый сервис"]
    R3["Библиотека<br/>или runtime"] --> I3["Собственная<br/>прикладная логика"]
  end

  class MODEL endgeInput
  class R1,R2,R3 endgeProcess
  class I1,I2,I3 endgePackage
```

Здесь стрелки показывают использование модели. Общее описание развивается в одном месте и поставляется потребителям в согласованных версиях. Приложения сохраняют собственные реализации, а различия между их сценариями задаются явно. Новая версия модели может потребовать обновления потребителя, если она использует ещё не поддерживаемую им возможность.

Ветка сервиса показывает принцип подключения совместимого потребителя. Для другой технологии требуется реализовать соответствующую библиотеку или runtime; наличие общей модели само по себе не добавляет её исполнение в произвольный микросервис.
