...
::: info Внешние права
Backend поддерживает `endge-access.yaml` и переопределение через `ACCESS_CONFIG_FILE`. Валидный файл включает внешнее управление назначениями; для него нужны OIDC и согласованный маппинг claims. Отдельный диалог внешних прав в frontend реализуется независимо от backend.
:::
