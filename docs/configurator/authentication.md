...
## Адаптеры профилей приложения

В Core реализованы пять встроенных адаптеров. Они настраиваются документами `AuthProfile` в конфигураторе и обслуживают обращения приложения к API:

| Адаптер | Что передаёт или получает |
| --- | --- |
| [OIDC](./authentication/oidc#runtime-profile) | Пользовательскую сессию через Authorization Code + PKCE |
| [Bearer](./authentication/bearer) | Готовый токен в `Authorization: Bearer …` |
| [Basic Auth](./authentication/basic) | Имя и пароль в `Authorization: Basic …` |
| [OAuth2 Client Credentials](./authentication/oauth2-client-credentials) | Access token по `clientId` и `clientSecret` |
| [OAuth2 Password](./authentication/oauth2-password) | Access token по имени и паролю пользователя |

Общий формат, переменные и хранение сессий описаны в разделе [Профили](./authentication/profiles). Наличие адаптера профиля не добавляет такой же способ входа в Service Backend и не назначает роли конфигуратора.
...
Для входа в конфигуратор сейчас существуют OIDC и dev-режим; для runtime-интеграций — пять [адаптеров профилей](./authentication/profiles). SAML и LDAP среди встроенных адаптеров отсутствуют.
