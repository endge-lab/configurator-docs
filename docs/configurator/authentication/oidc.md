...
Если `abac` есть только в ID token, настройте выдачу нужного claim в access token на стороне провайдера. Маппер должен получать актуальные данные и при обычной выдаче, и при refresh.

Механизм одинаков для стандартных массивов ролей Keycloak и пользовательского дерева `abac`: файл задаёт точный путь и условие. Он не интерпретирует произвольное дерево как готовые административные права Endge.

## Проект: пример данных для маппинга

Ниже искусственный **фрагмент claims**, а не JWT, который можно отправить API. Служебные поля проверки токена опущены. Названия прав специально относятся к конфигуратору:
...
С [основным YAML-примером](./access-configuration#yaml-example) этот фрагмент даёт Editor в workspace `operations`: совпадают правила Viewer и Editor, внутри одной области выбирается более высокая роль. Platform Admin не назначается, потому что `platform-admin` отсутствует.

Для `abac.flight.edit`, `businessRules.edit` или `resourceGroup: ["*"]` значение в конфигураторе не предполагается автоматически. Нужен отдельный согласованный маппинг. Роли клиента `realm-management` также не дают Platform Admin Endge без явно заданного правила.
...
Для приведённого выше фрагмента claims совпадут только Viewer и Editor. Чтобы получить Admin workspace, провайдер должен добавить `operations-admin` в массив ролей клиента; для Platform Admin — `platform-admin`.
...
## Профиль приложения {#runtime-profile}

OIDC также существует как встроенный адаптер `AuthProfile` для конечного приложения на Core. Этот профиль и описанная выше cookie-сессия конфигуратора настраиваются независимо: здесь токенами управляет приложение в браузере.
...
Подключённый источник предоставляет Core токены и их обновление. Без действующей сессии требуется интерактивный вход; адаптер не может получить пользовательский токен только из issuer и client ID. Настройки хранения и выбора профиля в запросах см. в разделе [Профили](./profiles).
