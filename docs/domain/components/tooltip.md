...
Базовое поведение задаётся во вкладке «Тултипы» конфигурации и наследуется от
Workspace через выбранные документы активных фасетов в порядке `RFacet.position`:
