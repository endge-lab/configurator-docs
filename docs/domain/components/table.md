...
Оба UI-адаптера применяют один и тот же Source default. В vue-shadcn пользователь
может затем менять видимость через column manager. Native Vue/RevoGrid также
скрывает перечисленные колонки и принимает runtime visibility actions, хотя его
встроенный UI управления отличается.
